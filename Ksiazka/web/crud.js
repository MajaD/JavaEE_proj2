...
// Model for Ksiazka entity
    models.Ksiazka = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Ksiazka/webresources/baza.ksiazka/",
        idAttribute: 'id',
        defaults: {
            tytul: "",
            autor: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Ksiazka/webresources/baza.ksiazka/';
            }
    ...
    // Collection class for Ksiazka entities
    models.KsiazkaCollection = Backbone.Collection.extend({
        model: models.Ksiazka,
        url: "http://localhost:8080/Ksiazka/webresources/baza.ksiazka/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.KsiazkaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-ksiazka-list-itemis template identifier for item
                        templateName: '#tpl-ksiazka-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.ksiazka = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.ksiazka,
                    // tpl-ksiazka-details is template identifier for chosen model element
                    templateName: '#tpl-ksiazka-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Ksiazka();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-ksiazka-details is a template identifier for chosen model element
                templateName: '#tpl-ksiazka-details',
